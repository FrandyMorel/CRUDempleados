...
export function EditarEmpleado(){
    ...
    const obternerEmpleado = async() => {
      const response = await fetch(`${appsettings.appUrl}Empleado/Obtener/${id}`)
      if(response.ok){
    ...
    const guardar = async () => {
          const response = await fetch(`${appsettings.appUrl}Empleado/Editar`,{
            method: 'PUT',
            headers: {
    ...
    return (
      ...
      <h1 className="flex justify-center text-4xl font-bold mt-8">Editar empleado</h1>
